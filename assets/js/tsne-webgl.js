...
var imageData = {};
var imageDataKeys = [];
selectedBrands = 'Observation|ParkRoadAmerica|WeddingsAreMayhem|TinCan|RingBearer';
...
function loadData() {
  get('brand_counts.json', function(data) {
    ...
    brandCounts = [
    {x: "Observation", value: brandCountsHolder["Observation"]},
    {x: "ParkRoadAmerica", value: brandCountsHolder["ParkRoadAmerica"]},
    {x: "TinCan", value: brandCountsHolder["TinCan"]},
    {x: "RingBearer", value: brandCountsHolder["RingBearer"]},
    {x: "WeddingsAreMayhem", value: brandCountsHolder["WeddingsAreMayhem"]}]
  })    
  ...
  get('plot_data.json', function(data_orig) {
    ...
    for(var i=0; i<dataHold.positions.length; i++){
        //console.log(dataHold.positions[i][0].substring(8, 11));
        if (selectedBrands.includes(dataHold.positions[i][0].substring(8, 11))) {
            dataPositions.push([dataHold.positions[i][0], dataHold.positions[i][1]/4, dataHold.positions[i][2]/4, dataHold.positions[i][3], dataHold.positions[i][4]]);
            //console.log(dataHold.positions[i]);
        } else{
            dataPositions.push([dataHold.positions[i][0], dataHold.positions[i][1]/2, dataHold.positions[i][2]/2, 0, 0])
        }
...
function addWindowEventListeners() {
  ...
  window.addEventListener('hashchange', function(e) {
    ...
    marker.XPos = (((coords.x + 4900)/(3207.5 + 4900)) * 300) - (marker.Width / 2);
    marker.YPos = (193 - ((coords.y + 4652)/(2918 + 4652)) * 193) - marker.Height;
    ...
    $("#brandname").html('Creative Spot: ' + imageBrandName);
    ...
    $("#brandpercentage").html('% of Spot in This Cluster: ' + (brandPercentage * 100).toFixed(1) + "%"); 
                                               
    clusterCounts = [
    {x: "Observation", value: thisClusterData["Observation"]},
    {x: "ParkRoadAmerica", value: thisClusterData["ParkRoadAmerica"]},
    {x: "TinCan", value: thisClusterData["TinCan"]},
    {x: "RingBearer", value: thisClusterData["RingBearer"]},
    {x: "WeddingsAreMayhem", value: thisClusterData["WeddingsAreMayhem"]}]
...
mapSprite.src = "assets/images/Allstate TV Full Map.png";
